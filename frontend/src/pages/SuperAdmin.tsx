...
const SuperAdmin: React.FC = () => {
  ...
  const [links, setLinks] = useState({ link_scanner: '', link_printer: '', link_display: '', transfer_alias: '' });
  ...
  const fetchData = async () => {
    try {
      ...
      configPrices?.forEach(cfg => {
        ...
        if (cfg.key === 'transfer_alias') transferAlias = cfg.value || '';
      });
  ...
  const saveLinks = async () => {
    try {
      ...
      const { error: errAlias } = await supabase
        .from('SystemConfig')
        .upsert({ key: 'transfer_alias', value: links.transfer_alias });

      if (errAlias) throw errAlias;

      setFeedbackMessage({ type: 'success', text: 'Configuración de hardware y alias de pago actualizada con éxito.' });
      setTimeout(() => setFeedbackMessage(null), 4000);
    } catch (error: any) {
  ...
  const [activateModal, setActivateModal] = useState<{ tenantId: string; tenantName: string; currentSubExpiresAt: string | null } | null>(null);
  const [activateDays, setActivateDays] = useState<string>('30');
  const [activatePlan, setActivatePlan] = useState<'STANDARD' | 'PRO'>('STANDARD');

  const openActivateModal = (tenantId: string, tenantName: string, currentPlan: string, currentSubExpiresAt: string | null) => {
    setActivateDays('30');
    setActivatePlan((currentPlan === 'PRO' ? 'PRO' : 'STANDARD') as 'STANDARD' | 'PRO');
    setActivateModal({ tenantId, tenantName, currentSubExpiresAt });
  };
  ...
  const handleToggleTenantStatus = async (tenantId: string, subActive: boolean, days?: number, plan?: string) => {
    try {
      ...
      let newStatus = subActive !== undefined ? subActive : !tenant.subActive;
      
      // Calculate new expiry — supports positive (agregar) and negative (descontar) days
      ...
      if (days !== undefined) {
        ...
        if (numDays !== 0) {
          ...
          // Auto-desactivar si la nueva fecha queda en el pasado
      ...
      const adjustedDays = days !== undefined ? parseInt(String(days), 10) : 0;
      const daysMsg = adjustedDays !== 0
        ? ` (${adjustedDays > 0 ? '+' : ''}${adjustedDays} días → vence ${new Date(newExpiresAt!).toLocaleDateString('es-AR')})`
        : '';
  ...
  return (
    <div className="p-8 space-y-8 bg-slate-50 dark:bg-slate-950 min-h-screen overflow-y-auto pb-24">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[40px] border border-slate-200 dark:border-slate-800 p-8 shadow-sm space-y-6">
        ...
        <div className="space-y-2">
          <label className="text-xs font-black uppercase text-slate-400 tracking-wider">Alias de Transferencia / CBU</label>
          <input
            type="text"
            value={links.transfer_alias}
            onChange={(e) => setLinks(prev => ({ ...prev, transfer_alias: e.target.value }))}
            className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl py-4 px-4 font-bold text-slate-900 dark:text-white text-sm focus:outline-none focus:border-blue-500 transition-colors"
            placeholder="ej: horacio.asa o CVU/CBU completo"
          />
          <p className="text-[10px] text-slate-400">Este alias se muestra a los clientes cuando reportan un pago por transferencia bancaria.</p>
        </div>
        ...
        <div className="flex justify-end pt-4 border-t border-slate-100 dark:border-slate-800">
          <button 
            ...
            className="bg-blue-600 hover:bg-blue-700 text-white font-black px-8 py-4 rounded-2xl flex items-center gap-2 transition-all hover:scale-[1.02] active:scale-95 disabled:opacity-50"
          >
            <Save size={20} /> {savingLinks ? 'Guardando...' : 'Guardar Configuración'}
          </button>
        </div>
      </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-950 border-b border-slate-100 dark:border-slate-800">
                  ...
                  <th className="p-4 text-xs font-black uppercase tracking-wider text-slate-400 text-center">Ventas</th>
                  <th className="p-4 text-xs font-black uppercase tracking-wider text-slate-400 text-center">Productos</th>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {filteredTenants.map((t) => (
                  <tr key={t.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors">
                    ...
                    <td className="p-4 text-center">
                      <span className="font-mono font-black text-sm text-slate-700 dark:text-slate-300">
                        {t.productCount}
                      </span>
                    </td>
                    ...
                    <td className="p-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        {!t.subActive ? (
                          <button
                            onClick={() => openActivateModal(t.id, t.name, t.plan, t.subExpiresAt)}
      ...
      {/* Activation / Adjustment Modal */}
      {activateModal && (() => {
        const numDays = parseInt(activateDays || '0');
        const isSubtracting = numDays < 0;
        const baseDate = (activateModal.currentSubExpiresAt && new Date(activateModal.currentSubExpiresAt) > new Date())
          ? new Date(activateModal.currentSubExpiresAt)
          : new Date();
        const previewDate = new Date(baseDate);
        if (numDays !== 0) previewDate.setDate(previewDate.getDate() + numDays);
        const previewInPast = numDays !== 0 && previewDate <= new Date();
              ...
              <div className={`p-6 text-white flex justify-between items-start ${
                ...
                <div>
                  <p className="text-[10px] font-black uppercase tracking-widest text-white/80 mb-1">
                    {isSubtracting ? 'Corregir / Descontar Días' : 'Activar / Extender Suscripción'}
                  </p>
                  <h3 className="text-xl font-black">{activateModal.tenantName}</h3>
                  {activateModal.currentSubExpiresAt && (
                    <p className="text-white/70 text-xs mt-1">
                      Vence actualmente: {new Date(activateModal.currentSubExpiresAt).toLocaleDateString('es-AR')}
                    </p>
                  )}
                </div>
              ...
              <div className="p-6 space-y-5">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-black uppercase text-slate-400 tracking-wider flex items-center gap-1.5">
                    <Calendar size={12} /> Ajuste de días
                  </label>

                  {/* Positive presets */}
                  <p className="text-[10px] font-black text-emerald-500 uppercase tracking-wider">Agregar días</p>
                  <div className="flex gap-2">
                    {['7', '15', '30', '60', '90'].map(d => (
                      <button
                        key={`+${d}`}
                  ...
                  {/* Negative presets */}
                  <p className="text-[10px] font-black text-red-400 uppercase tracking-wider">Descontar días (corrección)</p>
                  <div className="flex gap-2">
                    {['-7', '-15', '-30'].map(d => (
                      <button
                        key={d}
                        ...
                        className={`flex-1 py-2 rounded-xl font-black text-xs border transition-all ${
                          activateDays === d
                            ...
                            : 'bg-slate-50 dark:bg-slate-950 border-red-200 dark:border-red-900/40 text-red-400 hover:border-red-400'
                        }`}
                      >
                        {d}d
                      </button>
                  ...
                  <input
                    ...
                    placeholder="Positivo = agregar  /  Negativo = descontar"
                  />

                  {numDays === 0 ? (
                    <p className="text-xs text-amber-600 dark:text-amber-400 font-bold text-center">
                      ⚠ Con 0 días solo se cambia el plan, sin alterar la fecha.
                    </p>
                  ) : previewInPast ? (
                    <p className="text-xs text-red-500 font-black text-center">
                      ⚠ Nuevo vencimiento: {previewDate.toLocaleDateString('es-AR')} → fecha pasada → el comercio quedará <strong>DESACTIVADO</strong>
                    </p>
                  ) : (
                    <p className={`text-xs font-bold text-center ${isSubtracting ? 'text-orange-400' : 'text-slate-400'}`}>
                      Nuevo vencimiento: {previewDate.toLocaleDateString('es-AR', { day: '2-digit', month: '2-digit', year: 'numeric' })}
                    </p>
                  )}
                </div>
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button
                    ...
                    className={`flex-1 ${
                      isSubtracting
                        ? 'bg-gradient-to-r from-orange-600 to-red-600 shadow-red-600/20'
                        : 'bg-gradient-to-r from-emerald-600 to-teal-600 shadow-emerald-600/20'
                    } hover:opacity-90 text-white py-3.5 rounded-2xl font-black text-sm transition-all shadow-xl active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2`}
                  >
                    ...
                    {togglingTenantId === activateModal.tenantId
                      ? 'Procesando...'
                      : isSubtracting ? 'Aplicar Corrección' : 'Activar Ahora'}
                  </button>
                </div>
              </div>
            </div>
